...
public:
	...
	///iostream
	//cout.operator<<(int, char, string....)
	friend ostream& operator<<(ostream& out, Student& s);//cout<<s1 -> cout.operator<<(s1)
	...
	friend int operator+(int i, Student&);

	//friend int operator+(Student&,int i);
	friend Student operator+(Student&, float i);
	/*int operator+(int i) {
		return this->varsta + i;
	}*/

	Student operator+(const Student& s) {//s1+s2
		Student copie = *this;
		copie.varsta += s.varsta;
		return copie;
	}
...
Student operator+(Student& s, float i) {
	Student copie = s;
	copie.varsta += (int)i;
	return copie;
}
int operator+(int i, Student& s) {
	return s.varsta+i;
}
//int operator+(Student&s, int i) {
//	return s.varsta + i;
//}
...
int main() {
	...
	//	cout << endl << endl << j;
		...
		//++
		int ab = a + b;
		int j= 2 + s1;//j.operator=(2.operator+(s1))
		//  
		cout << s1;
		cout << "\n" << j;
		//j = s1 + 2+s1;//s1.operator+(2)
	
		cout << "\n" << j;
		float k = (float)2;
		Student s9 = s1 + s2;//student<-s1.operator+(student)
		cout << s1;
		cout << s2;
		cout << s9;
